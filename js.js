...
const checkShopName = (name) => {
  const namePart = name.match(/_.+\s.+\s/g);
...
document.querySelector('#commonReport').onchange = function () {
  ...
  reader.onload = function (progressEvent) {
    ...
    // shopsReports.forEach(report => report.region === 'Moscow' ? shopsMoscowForCalc.push(report) : shopsRegionForCalc.push(report));
...
document.getElementById('download').onclick = function () {
  ...
  //SUMMARY REGION
  csv += csv += total(shopsRegionForCalc);
  csv += `\n`;
  //TOTAL 
